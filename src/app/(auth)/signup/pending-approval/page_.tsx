...
import { AlertCircle, CheckCircle2, Copy, BookOpen, MessageSquare, HelpCircle, Clock } from 'lucide-react';
...
export default function PendingApprovalPage() {
  ...
  useEffect(() => {
    if (!token) {
      setError('No tracking tracking ID');
      setLoading(false);
      return;
    }
    ...
    async function checkApprovalStatus() {
      try {
        const response = await fetch(`/api/driver/approval-status?token=${token}`);
  ...
  function copyTrackingNumber() {
    ...
    const trackingUrl = `${window.location.origin}/pending-approval?token=${token}`;
        ...
        <Card className="w-full max-w-md border-destructive/20">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
              <Button className="w-full" onClick={() => router.refresh()}>Retry</Button>
              <Link href="/" passHref>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-3 py-4 px-4 rounded-lg bg-muted/20 border border-border/50">
              ...
              <button
                ...
                className="w-full flex items-center gap-2 p-3 rounded-lg bg-background border border-border/50 hover:border-primary/50 hover:bg-muted/50 transition-all group"
              >
